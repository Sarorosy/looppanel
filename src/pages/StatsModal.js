...
const StatsModal = ({ onClose, dayData, activeTab = "submitted" }) => {
  ...
  const tabs = [
    { key: "submitted", label: "Submitted", data: dayData?.submitted_quote_ids || [] },
    { key: "discount", label: "Discount Submitted", data: dayData?.discount_quote_ids || [] },
  ];
      ...
      >
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {tabs.find((t) => t.key === tab)?.data.map((id, idx) => (
              <div
                key={idx}
