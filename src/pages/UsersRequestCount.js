...
const UsersRequestCount = ({ onClose }) => {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:5000/api/scope/getAllInstaCrmusers', {
        method: 'POST',
  ...
  const handleFetchCount = async () => {
    try {
      ...
      setFetching(true);
      const response = await fetch('http://localhost:5000/api/scope/getUserRequestsCount', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
