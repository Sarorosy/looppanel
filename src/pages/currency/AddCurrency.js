...
export default function AddCurrency({ onClose, after }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `https://loopback-skci.onrender.com/api/helper/currency/create`,
        {
          method: "POST",
      ...
      toast.success("Cuurency Added successfully!");
      after();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-[#224d68] text-white">
        <h2 className="text-[16px] font-semibold">Add Currency</h2>
        <button
      ...
      <form className="p-4 space-y-4">
        ...
        <div className="flex justify-end">
          ...
          >
            {loading ? "Adding..." : "Add Currency"}<ChevronsRightIcon size={11} className="" />
          </button>
        </div>
      </form>
    </motion.div>
