...
export default function ManageCurrency({ onClose }) {
  ...
  const fetchCurrency = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://loopback-skci.onrender.com/api/helper/allcurrency", {
        method: "GET",
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `https://loopback-skci.onrender.com/api/helper/currency/delete/${selectedCurrency.id}`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
        }
