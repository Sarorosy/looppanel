...
export default function ManageRequirement({ onClose }) {
  ...
  const fetchRequirements = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://loopback-skci.onrender.com/api/helper/allrequirements", {
        method: "GET",
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `https://loopback-skci.onrender.com/api/helper/requirement/delete/${selectedRequirement.id}`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
        }
